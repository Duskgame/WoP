...
@onready var player_damage_mod: SpinBox = $VBoxContainer/HBoxContainer6/PlayerDamageModifier
...
func _ready() -> void:
	...
	player_damage_mod.value = State.damage_modifier
...
func _on_player_damage_modifier_value_changed(value: float) -> void:
	State.damage_modifier = value
